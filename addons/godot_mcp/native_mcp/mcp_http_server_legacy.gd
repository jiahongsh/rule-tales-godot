...
func start() -> bool:
	...
	# This project only permits local editor automation. TCPServer.listen()
	# defaults to "*", which would expose the unauthenticated MCP endpoint on
	# every network interface. Bind explicitly to IPv4 loopback instead.
	var error: Error = _tcp_server.listen(_port, "127.0.0.1")
	if error != OK:
